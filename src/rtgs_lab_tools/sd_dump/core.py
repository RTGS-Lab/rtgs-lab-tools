...
def find_particle_device() -> Optional[str]:
    ...
    for port in ports:
        ...
        if any(keyword in (port.description or "").lower() for keyword in 
               ["particle", "photon", "electron", "boron", "argon", "xenon"]):
            return port.device
...
def send_dump_command(ser: serial.Serial, recent_count: Optional[int] = None) -> bool:
    ...
    if command_echoed:
        logger.warning("Command was acknowledged but SD dump didn't start within timeout")
    
    return False


def receive_sd_dump(ser: serial.Serial, output_dir: Path, logger_func: callable = None) -> Tuple[bool, dict]:
    ...
    try:
        while True:
            ...
            if line.startswith("RECENT_COUNT:"):
                ...
                log(f"Filtering files: only most recent {recent_count} files of each type")
                ...
                log(f"Starting file {file_num}/{total_files}: {full_path} ({file_size} bytes)")
                
                if file_progress_bar:
                    file_progress_bar.close()
                file_progress_bar = tqdm(total=total_chunks, desc=f"Chunks ({file_path.name})", 
                ...
                if calculated_file_crc == expected_file_crc:
                    ...
                    log(f"File saved: {current_file['local_path']} ({len(current_file['data'])} bytes)")
                else:
...
def dump_sd_card(
    ...
    try:
        ...
        if not port:
            ...
            if not port:
                raise SDDumpError("No Particle device found. Please specify --port manually.")
        ...
        try:
            ...
            if not skip_trigger:
                if not wait_for_trigger_response(ser, timeout):
                    raise SDDumpError("Failed to enter command mode. Try using --skip-trigger if device is already in command mode, or power cycle the device.")
            else:
            ...
            if not success:
                raise SDDumpError(f"SD dump failed: {results.get('error', 'Unknown error')}")
...
def list_available_ports() -> list:
    ...
    for port in ports:
        particle_device = (
            port.vid == 0x2B04 or 
            any(keyword in (port.description or "").lower() for keyword in 
                ["particle", "photon", "electron", "boron", "argon", "xenon"])
        )
...
def send_write_command(ser: serial.Serial, filename: str) -> bool:
    ...
    if command_echoed:
        logger.warning("Command was acknowledged but SD write didn't start within timeout")
    
    return False


def send_file_over_serial(ser: serial.Serial, file_path: Path, filename: str, logger_func: callable = None) -> Tuple[bool, dict]:
    ...
    try:
        ...
        progress_bar = tqdm(total=total_chunks, desc=f"Uploading {filename}", unit="chunk")
        bytes_sent = 0
        ...
        try:
            for chunk_num in range(1, total_chunks + 1):
                ...
                while retry_count < max_retries:
                    ...
                    while (time.time() - start_time) < 30.0:  # 30 second timeout per chunk
                        if ser.in_waiting > 0:
                            response = ser.readline().decode('utf-8', errors='ignore').strip()
                    ...
                    if ack_received:
                        ...
                        if retry_count < max_retries:
                            log(f"Timeout waiting for ACK on chunk {chunk_num}, retrying...")
                            # Resend the chunk
                            ser.write(chunk_msg.encode())
                            ser.flush()
                
                if retry_count >= max_retries:
                    return False, {"error": f"Failed to send chunk {chunk_num} after {max_retries} attempts"}
...
def write_file_to_sd(
    ...
    try:
        ...
        try:
            ...
            success, results = send_file_over_serial(ser, input_path, filename, logger_func)
            
            if not success:
                raise SDDumpError(f"SD write failed: {results.get('error', 'Unknown error')}")
