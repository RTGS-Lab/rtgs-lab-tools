...
@click.option('--output-dir', '-o', type=click.Path(), default='./sd_dump_output', 
              help='Output directory (default: ./sd_dump_output)')
@click.option('--timeout', default=60, help='Connection timeout in seconds (default: 60)')
@click.option('--skip-trigger', is_flag=True, help='Skip trigger phase (device already in command mode)')
@click.option('--recent', '-r', type=int, help='Only dump the most recent N files of each type (data, error, diag, meta)')
@add_common_options
@click.pass_context
...
def dump(
    ...
    try:
        ...
        if success:
            click.echo(f"Successfully dumped {results['files_processed']} files ({results['bytes_transferred']} bytes)")
            ...
            if recent:
                additional_sections["Filter Applied"] = f"- **Recent Files**: Only most recent {recent} files of each type"
            ...
            raise RTGSLabToolsError(f"SD dump failed: {results.get('error', 'Unknown error')}")
...
@click.option('--filename', '-f', help='Filename to save on device (defaults to original filename)')
@click.option('--port', '-p', help='Serial port (auto-detected if not specified)')
@click.option('--baudrate', '-b', default=1000000, help='Baud rate (default: 1000000)')
@click.option('--timeout', default=60, help='Connection timeout in seconds (default: 60)')
@click.option('--skip-trigger', is_flag=True, help='Skip trigger phase (device already in command mode)')
...
def write(
    ...
    try:
        ...
        if success:
            click.echo(f"Successfully uploaded {results['bytes_sent']} bytes to {results['device_filename']}")
            ...
            raise RTGSLabToolsError(f"SD write failed: {results.get('error', 'Unknown error')}")
...
def list_ports(ctx):
    ...
    for port_info in ports:
        ...
        click.echo(f"  {port_info['device']} - {port_info['description']} {particle_indicator}")
