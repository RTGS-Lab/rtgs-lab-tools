...
def update():
    ...
    if click.confirm("Do you want to update now?"):
        if run_install_script():
            ...
            click.echo("Please restart your terminal or reactivate your virtual environment.")
        else:
            click.echo(click.style("Update failed. Please check the output above.", fg="red"))
            sys.exit(1)
    else:
...
def version():
    ...
    if version_info['latest']:
        ...
        if version_info['update_available']:
            click.echo(click.style("Update available! Run 'rtgs core update' to update.", fg="yellow"))
    else:
