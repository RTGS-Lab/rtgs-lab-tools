...
def extract_array_value(
    value: Union[str, list, float, int], index: int
...
def load_and_prepare_data(
    file_path: str, packet_types: str = "all", cli_ctx=None, auto_parse: bool = False
    ...
    if data_type == "raw":
        ...
        if not auto_parse:
            ...
            if not click.confirm(
                ...
                click.echo(
                    "   - First run: rtgs-lab-tools data-parser parse your_raw_file.csv"
                )
        ...
        if not auto_parse:
            ...
            click.echo(
                f"✅ Data parsed successfully! Saved to: {parsed_file_path.name}"
            )
            if cli_ctx:
        ...
        if cli_ctx:
            ...
            if parsing_results["packet_types"] != "all":
                additional_sections["Packet Types"] = (
                    f"- **Filtered Types**: {parsing_results['packet_types']}"
                )
...
def get_available_measurements(df: pd.DataFrame) -> Dict[str, set]:
    ...
    for node_id in df["node_id"].unique():
        ...
        for measurement_name in node_data["measurement_name"].dropna().unique():
            ...
            measurement_rows = node_data[
                node_data["measurement_name"] == measurement_name
            ]
...
def filter_parsed_data(
    df: pd.DataFrame, measurement_spec: str, node_ids: Optional[list] = None
) -> pd.DataFrame:
        ...
        if "measurement_path" in filtered_df.columns:
            filtered_df["measurement_path"] = filtered_df["measurement_path"].apply(
                lambda x: (
                    f"{x}[{array_index}]"
                    if pd.notna(x)
                    else f"{measurement_name}[{array_index}]"
                )
            )
