...
class MetadataV2Parser(EventParser):
    ...
    def parse(self, raw_data: pd.Series) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if not data:
                self._log_parsing_issue(f"Invalid metadata format for record {raw_data.get('id')}")
                return []

            # Process the metadata fields
            self._process_metadata_fields(
                data=data, path_prefix="", common_fields=common, result=result
            )

        except Exception as e:
            self._log_parsing_issue(f"Error parsing metadata in record {raw_data.get('id')}: {e}")
    ...
    def _process_metadata_fields(
        ...
        if "Devices" in metadata_section and isinstance(
            metadata_section["Devices"], list
        ):
    ...
    def _process_device_metadata(
        ...
        for key, value in device_data.items():
            ...
            if isinstance(value, dict):
                ...
                if any(
                    x in key.lower()
                    for x in ["firmware", "hardware", "version", "schema"]
                ):
                    unit = "version"
