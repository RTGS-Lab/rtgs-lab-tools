...
def login(headless):
    ...
    if status["authenticated"] and status["secret_manager_access"]:
        console.print(f"✅ Already authenticated as: [bold green]{status['user']}[/bold green]")
        ...
        console.print()
        console.print("You're already set up! Run [bold blue]rtgs sensing-data list-projects[/bold blue] to get started.")
        return
    
    # Show headless mode info
    if headless:
        console.print("🖥️  [bold yellow]Headless Mode:[/bold yellow] Authentication will not open a browser")
    ...
    if headless:
        ...
        with console.status("[bold blue]Authenticating with Google Cloud...", spinner="dots"):
    ...
    if result["success"]:
        ...
        if result.get("secret_manager_access"):
            ...
            console.print("⚠️  Secret Manager access: [bold yellow]Limited[/bold yellow]")
...
def status():
    ...
    if auth_status["gcloud_installed"]:
        ...
        table.add_row("gcloud CLI", "❌ Missing", "Run 'rtgs auth login' for installation instructions")
    ...
    if auth_status["project"]:
        ...
        table.add_row("Project", "⚠️  Not set", "Run 'gcloud config set project PROJECT_ID'")
    ...
    if auth_status["secret_manager_access"]:
        ...
        table.add_row("Secret Manager", "❌ No access", "Contact admin for IAM permissions")
    else:
    ...
    if not auth_status["authenticated"]:
        console.print("[bold yellow]Recommendation:[/bold yellow] Run [bold]rtgs auth login[/bold] to authenticate")
    elif not auth_status["secret_manager_access"]:
        console.print("[bold yellow]Recommendation:[/bold yellow] Contact your administrator for Secret Manager permissions")
    elif not auth_status["project"]:
        console.print("[bold yellow]Recommendation:[/bold yellow] Set your project with [bold]gcloud config set project PROJECT_ID[/bold]")
    else:
...
def logout():
    ...
    if click.confirm("Are you sure you want to logout?"):
        ...
        if result["success"]:
            ...
            console.print("Your application will now fall back to .env file credentials")
        else:
