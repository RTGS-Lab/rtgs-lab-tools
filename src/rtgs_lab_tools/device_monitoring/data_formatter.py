...
def format_data(filepath):
    ...
    node_filter = (
        "all"  # Use "all" to get counters for all nodes, or specify a list of node IDs
    )
    ...
    formatted_data = {
        node: {"errors": error_counts, "battery_voltage": None}
...
def filter_sensor_data(df):
    ...
    if "measurement_name" in df.columns:
        temperature_data = df[
            df["measurement_name"].str.contains("Temperature", na=False)
        ].copy()
        ...
        if agg_dict:
            ...
            if "node_id" in daily_summary.columns:
                daily_summary = daily_summary.rename(
                    columns={"node_id": "total_records"}
                )
            if "device_type" in daily_summary.columns:
                daily_summary = daily_summary.rename(
                    columns={"device_type": "unique_devices"}
                )
...
def analyze_numeric_values(df):
    ...
    if len(numeric_data) > 0:
        ...
        if "device_type" in numeric_data.columns:
            ...
            device_stats = numeric_data.groupby("device_type")["numeric_value"].agg(
...
def parse_metadata(df):
    ...
    if len(metadata_records) > 0:
        ...
        metadata_records["parsed_metadata"] = metadata_records["metadata"].apply(
            safe_parse_metadata
        )
        ...
        if parsed_count > 0:
            ...
            sample_metadata = metadata_records[
                metadata_records["parsed_metadata"].notna()
            ]["parsed_metadata"].iloc[0]
...
def create_visualizations(df):
    ...
    try:
        ...
        if "event_type" in df.columns:
            event_counts = df["event_type"].value_counts()
            axes[0, 0].pie(
                event_counts.values, labels=event_counts.index, autopct="%1.1f%%"
            )
        ...
        if "timestamp" in df.columns:
            try:
                ...
                axes[1, 1].text(
                    0.5, 0.5, f"Hour analysis error", ha="center", va="center"
                )
