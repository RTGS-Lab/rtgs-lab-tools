...
def calculate_reference_et(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['Sunset Hour Angle (rad)'] = np.arccos(-np.tan(df['Latitude_rad'])
                                              * np.tan(df['Solar Declination (rad)']))

    # Ra (Extraterrestrial Radiation) in MegaJoules per square meter per day (MJ/m²/day)
    df['Ra (MJ/m²/day)'] = (24 * 60 / np.pi) * 0.082 * df['Inverse Relative Distance Earth-Sun'] * \
        (df['Sunset Hour Angle (rad)'] * np.sin(df['Latitude_rad']) * np.sin(df['Solar Declination (rad)']) + \
            (np.cos(df['Latitude_rad']) * np.cos(df['Solar Declination (rad)']) * np.sin(df['Sunset Hour Angle (rad)']))
        )
    ...
    df['Rnl (MJ/m²/day)'] = (4.903e-9 * ((df['Tavg (°C)'] + 273.16)**4 + (df['Tmin_C'] + 273.16)**4) / 2) * \
    ...
    df['ETo (in/day)'] = (0.408 * df['Delta (kPa/°C)'] * df['Rn (MJ/m²/day)'] + \
        df['Gamma (kPa/°C)'] * (900 / (df['Tavg (°C)'] + 273)) * \
        df['U2 (m/s)'] * df['VPD (kPa)']) / (df['Delta (kPa/°C)'] + \
        df['Gamma (kPa/°C)'] * (1 + 0.34 * df['U2 (m/s)'])) * 0.0393701

    # ETr (Reference Evapotranspiration for grass) in inches per day (in/day)
    df['ETr (in/day)'] = (0.408 * df['Delta (kPa/°C)'] * \
        df['Rn (MJ/m²/day)'] + df['Gamma (kPa/°C)'] * (1600 / (df['Tavg (°C)'] + 273)) * \
        df['U2 (m/s)'] * df['VPD (kPa)']) / \
        (df['Delta (kPa/°C)'] + df['Gamma (kPa/°C)'] * (1 + 0.38 * df['U2 (m/s)'])) * 0.0393701

    # Clean up intermediate columns
    intermediate_cols = [
        'Julian Date (day)', 'U2 (m/s)', 'Rs (MJ/m²/day)', 'Tavg (°C)', 
        'Lambda (MJ/kg)', 'P (kPa)', 'Gamma (kPa/°C)', 'Delta (kPa/°C)', 
        'eᵒ(Tmax) (kPa)', 'eᵒ(Tmin) (kPa)', 'eᵒs (kPa)', 'eᵒa (kPa)', 
        'VPD (kPa)', 'Inverse Relative Distance Earth-Sun', 'Solar Declination (rad)',  
        'Sunset Hour Angle (rad)', 'Ra (MJ/m²/day)', 'Rso (MJ/m²/day)', 
        'Rns (MJ/m²/day)', 'Rnl (MJ/m²/day)', 'Rn (MJ/m²/day)'
    ]
