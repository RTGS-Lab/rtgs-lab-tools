...
def sample_raw_data():
    ...
    return pd.DataFrame({
        ...
        'event': ['temperature', 'humidity', 'temperature', 'humidity', 'temperature'],
        'message': ['{"temp": 22.5}', '{"humidity": 65}', '{"temp": 23.1}', '{"humidity": 62}', '{"temp": 21.8}'],
...
def mock_database_manager(mock_config, sample_raw_data, sample_projects):
    ...
    db_manager.get_nodes_for_project.return_value = pd.DataFrame({
        ...
        'deployment_date': pd.to_datetime(['2023-01-01', '2023-01-02', '2023-01-03'])
    })
    return db_manager
